...
function AdminUsersPage({ onBack }: { onBack: () => void }) {
  ...
  const [removeTarget, setRemoveTarget] = useState<CurrentUser | null>(null)
  const token = localStorage.getItem('landguard_access_token')
  const loadAccounts = () => fetch(`${apiUrl}/users`, { headers: { Authorization: `Bearer ${token}` } }).then((response) => response.ok ? response.json() : []).then(setAccounts)
  ...
  return <section className="workspace-page"><button className="back-button" onClick={onBack}>← Dashboard</button><div className="workspace-header"><div><h1>Users</h1><p>Only administrators can create officer and auditor accounts, or remove access for staff who have left.</p></div></div><div className="admin-user-layout"><form className="user-create-form" onSubmit={submit}><p className="eyebrow">Create account</p><div className="create-fields"><label>Full name<input required placeholder="e.g. Diane Uwimana" value={form.full_name} onChange={(event) => setForm({ ...form, full_name: event.target.value })} /></label><label>Username or email<input required type="email" placeholder="d.uwimana" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label>Temporary password<input required type="password" minLength={8} placeholder="Auto-generated or set manually" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /></label><label>Role<select value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })}><option value="VERIFICATION_OFFICER">Verification officer</option><option value="AUDITOR">Auditor</option></select></label></div><button className="login-button" type="submit">Create user</button>{message && <p className="form-message">{message}</p>}<small>The new user must change their password on first sign-in. This action is recorded in the audit log.</small></form><div className="accounts-panel"><div className="accounts-heading"><div><strong>Authorized accounts</strong><span>{accounts.length} active accounts</span></div></div>{accounts.map((account) => <div className="account-row" key={account.id}><div className="avatar">{initials(account.full_name)}</div><div className="account-name"><strong>{account.full_name}</strong><span>{account.email}{!account.is_active && ' (access revoked)'}</span></div><span className={`account-role account-role--${account.roles[0]?.toLowerCase()}`}>{displayRole(account.roles[0]).replace('Verification ', '')}</span><span className={account.is_active ? 'account-active' : 'account-inactive'}>{account.is_active ? '● Active' : '● Revoked'}</span><button className="account-icon" aria-label={`Disable ${account.full_name}`} onClick={() => setRemoveTarget(account)} disabled={!account.is_active || account.id === 0}><MinusCircle size={14} /></button><button className="account-icon account-icon--delete" aria-label={`Remove access for ${account.full_name}`} onClick={() => setRemoveTarget(account)} disabled={!account.is_active || account.id === 0}><Trash2 size={14} /></button></div>)}</div></div>{removeTarget && <div className="modal-backdrop"><div className="remove-modal"><div className="remove-icon"><Trash2 size={22} /></div><h2>Remove {removeTarget.full_name}?</h2><p>This account will immediately lose access to LandGuard AI. Their <strong>{displayRole(removeTarget.roles[0])}</strong> role and login will be revoked.</p><div className="remove-note">Their past actions stay in the audit log and case history. Removing the account does not delete records they created or reviewed.</div><div className="modal-actions"><button className="cancel-button" onClick={() => setRemoveTarget(null)}>Cancel</button><button className="remove-button" onClick={removeAccess}>Remove user</button></div></div></div>}</section>
}
